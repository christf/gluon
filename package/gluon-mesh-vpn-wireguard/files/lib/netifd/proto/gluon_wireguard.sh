...
proto_gluon_wireguard_setup() {
  ...
  if [[ $(uci get wireguard.mesh_vpn_backbone.enabled) -eq 1 ]]; then
    ip link del "$ifname"
    ip link add dev "$ifname" type wireguard
    ip link set mtu $(uci get network.$config.mtu) dev "$ifname"
    ...
    if [[ "$remoteport" == "FULL" ]]; then
      ...
      echo "error when setting up wireguard connection for $ifname - no response from broker" >&2
      ip link del "$ifname"
      exit 1
    fi

    gluon-wan wg set "$ifname" private-key /lib/gluon/mesh-vpn/wireguard peer $(uci get wireguard.$thispeer.key) endpoint $(uci get wireguard.$thispeer.remote):$remoteport allowed-ips ::/0 persistent-keepalive 25
